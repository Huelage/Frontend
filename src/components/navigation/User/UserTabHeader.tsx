...
import { getCart, getEntity, getThemeType, switchTheme } from "@api/slices/globalSlice";
...
const UserTabHeader = () => {
  ...
  const cartLength = useAppSelector(getCart).length;
      ...
      <View style={styles.headerBox}>
        <View style={styles.headerDetail}>
          <View style={[styles.headerImageBox, { backgroundColor: color.cardBg }]}>
            {!!entity.imgUrl ? (
              <Image testID="user image" style={styles.headerImage} source={{ uri: entity.imgUrl }} />
            ) : (
              <Text testID="user image text" style={[styles.headerImageText, { color: color.mainText }]}>{entity.firstName?.toUpperCase()[0]}</Text>
            )}
          </View>
          <Text style={[styles.headerName, { color: color.mainText }]}>{`${entity.firstName} ${entity.lastName}`}</Text>
        </View>
        <View style={styles.headerAction}>
          ...
          <TouchableOpacity testID="cart button" onPress={goToCart}>
            ...
            {cartLength ? (
              <View style={[styles.cartLengthBox, { backgroundColor: color.danger }]} testID="cart item length">
