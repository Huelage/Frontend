...
const SideOptionElement = ({ name, price, isSingle, groupId, optionSelected, increase, decrease }: SideOptionElementInterface) => {
  ...
  const handleExtra = () => {
    if (selected) {
      decrease(isSingle ? { name, price, groupId } : { name, price, groupId, quantity: 0 });
      setQuantity(0);
    } else {
  ...
  const increaseExtra = () => {
    increase(isSingle ? { name, price, groupId } : { name, price, groupId, quantity: quantity + 1 });
  ...
  return (
    <View style={styles.container}>
      ...
      {isSingle || quantity < 1 ? (
        <TouchableOpacity onPress={handleExtra} style={[styles.radioButton, { borderColor: color.mainGreen }]}>
